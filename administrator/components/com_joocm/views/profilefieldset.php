		...
		<form action="index.php?option=com_joocm" method="post" name="adminForm">
			...
			<thead><tr>
                ...
                <th nowrap="nowrap" width="90%">
                    <?php echo JHTML::_('grid.sort', 'COM_JOOCM_NAME', 'p.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_profilefieldset_view'); ?>
                </th>
                <th nowrap="nowrap" width="1%">
                    <?php echo JHTML::_('grid.sort', 'COM_JOOCM_PUBLISHED', 'p.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_profilefieldset_view'); ?>
                </th>
                <th nowrap="nowrap" width="8%">
                    <?php echo JHTML::_('grid.sort', 'COM_JOOCM_ORDER', 'p.ordering', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_profilefieldset_view'); ?>
                    <?php echo JHTML::_('grid.order', $rows, 'filesave.png', 'joocm_profilefieldset_saveorder'); ?>
                </th>
                <th nowrap="nowrap" width="1%">
                    <?php echo JHTML::_('grid.sort', 'COM_JOOCM_ID', 'p.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_profilefieldset_view'); ?>
                </th>
			</tr></thead>
			<tfoot><tr>
			...
			for ( $i=0, $n=count( $rows ); $i < $n; $i++ ) {
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td class="order">
						<span><?php echo $pageNav->orderUpIcon( $i, true, 'joocm_profilefieldset_orderup', 'COM_JOOCM_ORDERUP', true ); ?></span>
						<span><?php echo $pageNav->orderDownIcon( $i, $n, true, 'joocm_profilefieldset_orderdown', 'COM_JOOCM_ORDERDOWN', true ); ?></span>
	...
	function editProfileFieldSet(&$row, &$lists) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('COM_JOOCM_MSGFIELDREQUIRED', JText::_('COM_JOOCM_NAME'), JText::_('COM_JOOCM_PROFILEFIELDSET')); ?>");
			} else {
		...
		<form action="index.php" method="post" name="adminForm" id="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend><?php echo JText::_('COM_JOOCM_FIELDSETDETAILS'); ?></legend>
