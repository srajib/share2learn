...
if ($user->id==0 && !$this->params->get("remindemails",0) && $this->params->get("reminders",0)){
	...
	$link = JRoute::_("index.php?option=com_rsvppro&task=reminders.record&eventid=".$eventid."&Itemid=$Itemid",false);
...
<form action="'.$link.'"  method="post"  name="jevreminderform"   id="jevreminderform">';
		...
		if ($this->params->get("remindemails",0) && $user->id==0 ) {
			...
			if ($registry->getValue("showingemailaddress",false)){
				$html .= '
		...
		$html .='<label for="jevremind">'. ($this->row->hasrepetition()?JText::_( 'JEV_REMIND_ME_ALL_REPEATS' ):JText::_( 'JEV_REMIND_ME_THIS_EVENT' )).'</label>
