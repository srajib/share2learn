...
if (count($this->attendees) > 0)
{
	...
	if ($this->attendees && count($this->attendees) > 0)
	{
		...
		foreach ($this->attendees as $attendee)
		{
			...
			if ($user->id == $this->row->created_by() || JEVHelper::isAdminUser($user) || $this->params->get("showcf", 0))
			{
				...
				else if (JRequest::getCmd("jevtask")!="icalrepeat.detail") {
					$html .='';
				}
				...
				$html .="<tr class='jevattendeerow'><td $rowspan><div class='jevattendee'>" . $name . "</div></td>\n";
			}
			// Show the status
			if ($showstatus)
			{
				...
				$html .='<td align="center" '.$rowspan.'><img src="' . JURI::root() . $pluginpath.'assets/' . $img . '"  style="height:16px;" alt="' . $img . '" />';
			...
			if ($user->id == $this->row->created_by() || JEVHelper::isAdminUser($user) || $this->params->get("showcf", 0))
			{
				...
				if ($this->rsvpdata->template != "")
				{
					...
					if (is_int($xmlfile) || file_exists($xmlfile) )	{
						...
						$params = $params->renderToBasicArray('params', '_default', $attendee);
				...
				if ($attendee->guestcount > 0 && isset($params))
				{
	...
	if (count($this->attendees) > 0)
	{
		...
		if ($user->id == $this->row->created_by() || JEVHelper::isAdminUser($user) || $this->params->get("showcf", 0))
		{
			...
			if ($this->rsvpdata->template != "")
			{
				...
				if (is_int($xmlfile) || file_exists($xmlfile) )	{
					...
					$params = $jevparams->renderToBasicArray('params');
					foreach ($params as $param)
					{
						if ($param['formonly'] || !$param['showinlist'])
							continue;
		...
		if ($this->params->get("sortableattendees", 0) && JRequest::getCmd("jevtask")=="icalrepeat.detail")
		//if ($this->params->get("sortableattendees", 0) && !$hasguest)
		{
			$registry	=& JRegistry::getInstance("jevents");
			if (!$registry->getValue("calledthis",0)) {
				JHTML::script('mootable.js',$pluginpath. 'mootable/');
				JHTML::stylesheet('mootable.css', $pluginpath.'mootable/');
				$doc = JFactory::getDocument();
				$doc->addScriptDeclaration("window.addEvent('load', function(){if ($('jevattendees')) var mootable = new MooTable($('jevattendees'));});");
				$registry->setValue("calledthis",1);
			}
		}
	}
	if ($this->jomsocial)
