...
var MooTable = new Class({
	...
	_addBreak : function (){
		//var cell = new Element('div').injectInside( this.thead_tr ).addClass('break');
	},
	...
	_addHeader: function( value, opts ){
		...
		new Element('div', {html: value}).addClass('cell').injectInside( cell );
		var h = {
			element: cell,
			value: value,
			options: options
		...
		if( !h.options.fixedWidth ){
			var handle = new Element('div', {html: '&nbsp;'}).addClass('resize').injectInside( h.element );
			try {
	...
	_initDisplayOptions: function(){
		...
		this.headers.each( function( header ){
			...
			var label = new Element('label', {html:header.value}).setProperty('for',id).setProperty('htmlFor',id).injectInside(this.form);
			i++;
